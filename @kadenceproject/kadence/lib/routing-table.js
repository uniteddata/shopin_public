...
class RoutingTable extends Map {
  ...
  get allBuckets() {
    ...
    this.forEach(bucket => {
      bucket.length > 0
        ? bucket.something().forEach(contact => {
            // delete contact.privateKey;
            contact.contact.clients.length > 0 ? contact.contact.clients.forEach((client) => {
  ...
  getAllContacts(contacts) {
    ...
    return this.get(this.indexOf(contacts)).get(contacts);
  }
  ...
  getClosestContactsToKey(key, n = constants.K, exclusive = false) {
    ...
    function _addNearestFromBucket(bucket) {
      ...
      entries.splice(0, n - contactResults.size).forEach(([id, contact]) => {
