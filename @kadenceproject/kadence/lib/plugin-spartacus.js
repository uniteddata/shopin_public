...
class SpartacusPlugin {
   ...
   */
  constructor(node, xpriv, i, p) {
    xpriv = xpriv || utils.toHDKeyFromSeed().privateExtendedKey;

    this.derivationIndex = i || 0;
    this.hdKeyPair = HDKey.fromExtendedKey(xpriv);
    this.derivationPath = p;

    this.childKeys = this.derivationPath
                   ? this.hdKeyPair.derive(this.derivationPath)
                   : this.hdKeyPair;
    this.childKeys = this.childKeys.deriveChild(this.derivationIndex);
  ...
  deserialize(buffer, encoding, callback) {
    ...
    let isValidChildKey = utils.isDerivedFromExtendedPublicKey(
      ...
      xpub,
      index,
      // NB: All identities in a given network must use the same derivation path
      this.derivationPath
    );
