...
class EclipseIdentity extends EventEmitter {
   ...
   * @param {string} [path] - Key derivation path
   */
  constructor(xprv, index = 0, path) {
    super();
   ...
   */
  validate() {
    let parent = hdkey.fromExtendedKey(this.xprv);

    if (this.path) {
      parent = parent.derive(constants.HD_KEY_DERIVATION_PATH);
    }
